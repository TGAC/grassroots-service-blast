...
#define ALLOCATE_BLAST_SERVICE_JOB_TAGS (1)
	...
	if (blast_job_p)
		{
			...
			InitServiceJob (base_service_job_p, service_p, job_name_s, job_description_s, NULL, NULL, NULL, NULL, BSJ_TYPE_S);
	...
	if (job_json_p)
		{
			...
			if (blast_job_p)
				{
					GrassrootsServer *grassroots_p = GetGrassrootsServerFromService (config_p -> bsd_base_data.sd_service_p);
...
bool AddErrorToBlastServiceJob (BlastServiceJob *job_p)
{
	...
	if (log_s)
		{
			if (AddGeneralErrorMessageToServiceJob (& (job_p -> bsj_job), log_s))
				{
					success_flag = true;
				}
			else
