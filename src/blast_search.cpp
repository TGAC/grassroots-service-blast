...
{
public:
	bool ParseParameterSet (ParameterSet *param_set_p);

private:
	CommandLineArgs cbda_args;
...
bool CBlastDemoApplication :: ParseParameterSet (ParameterSet *param_set_p)
{
	if (param_set_p)
		{
			ParameterNode *node_p = param_set_p -> ps_params_p -> ll_head_p;
...
void CBlastDemoApplication::Init (void)
{
	...
	arg_desc->SetConstraint (
			"program",
			&(*new CArgAllow_Strings, "blastn", "megablast", "dc-megablast", "blastp", "blastx", "tblastn", "tblastx", "rpsblast"));

	arg_desc->AddDefaultKey ("db", "DataBase", "This is the name of the database",
													 CArgDescriptions::eString, "nr");

	arg_desc->AddDefaultKey ("in", "Queryfile", "A file with the query",
													 CArgDescriptions::eInputFile, "stdin");

	arg_desc->AddDefaultKey ("out", "Outputfile", "The output file",
...
int CBlastDemoApplication :: Run (void)
{
	...
	bool db_is_aa = (program == eBlastp || program == eBlastx || program == eRPSBlast || program == eRPSTblastn);
	...
	EMoleculeType mol_type = db_is_aa ? CSearchDatabase :: eBlastDbIsProtein : CSearchDatabase :: eBlastDbIsNucleotide;
	const CSearchDatabase target_db (args ["db"].AsString (), mol_type);
			...
			if (messages.size () > 0)
				{
					...
					ITERATE(vector<CRef<CSearchMessage> >, it, messages)cerr << (*it)->GetMessage() << endl;
				}
		}
